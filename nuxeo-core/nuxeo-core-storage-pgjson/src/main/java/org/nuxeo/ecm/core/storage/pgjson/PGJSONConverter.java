...
import org.nuxeo.ecm.core.storage.pgjson.PGType.PGTypedValue;
...
public class PGJSONConverter {
    ...
    public PGJSONConverter(Map<String, Type> allTypes) {
    ...
    public static class UpdateBuilder {
        ...
        public final List<PGTypedValue> values = new ArrayList<>();
        ...
        protected String buildDelta(String expr, PGType type, Delta delta) {
            values.add(new PGTypedValue(delta.getDeltaValue(), type));
            return expr + " + ?";
        }

        protected String buildValue(PGType type, Object value) {
            values.add(new PGTypedValue(value, type));
        ...
        protected String buildStateDiff(String expr, StateDiff stateDiff, boolean exprNeedsParens) {
            ...
            if (!set.isEmpty()) {
                buf.append(" || ?::jsonb");
                values.add(new PGTypedValue(set, TYPE_JSON));
            }
            if (!unset.isEmpty()) {
                if (unset.size() == 1) {
                    buf.append(" - '");
        ...
        protected String buildListDiff(String expr, PGType type, ListDiff listDiff, boolean exprNeedsParens) {
            if (listDiff.diff != null) {
                ...
                for (Object value : listDiff.diff) {
                    if (value instanceof StateDiff) {
                        ...
                        values.add(new PGTypedValue(value, TYPE_JSON));
                    }
                    i++;
                }
            }
            if (listDiff.rpush != null) {
                if (type == TYPE_JSON) {
                    ...
                    values.add(new PGTypedValue(array, type));
                    expr += " || ?";
                } else {
